...
class BleConnectionStore {
	...
	async connectRXTX() {
		...
		this.rxtxCharacteristic.addEventListener('characteristicvaluechanged', (event: Event) => {
			...
			if (data.byteLength === 6 && data[0] === 0xe2 && data[1] === 0xac) {
				const unix = data[2] | (data[3] << 8) | (data[4] << 16) | (data[5] << 24);
				const now = new Date();
				const localUnix = Math.floor((now.getTime() - now.getTimezoneOffset() * 60_000) / 1000);
				const diff = unix - localUnix;
				logStore.addLog(`[Time Test] Tag Local UNIX: ${unix}, Browser Local UNIX: ${localUnix}. Diff: ${diff} seconds.`);
				return;
			}
