	...
	<div class="collapse-content">
		<div class="prose max-w-none p-0">
			<div class="flex gap-3 flex-wrap">
				...
				<button
					class="btn btn-info"
					onclick={() => bleConnectionStore.sendRxTxCommand('e2ac')}
					disabled={!bleConnectionStore.connected || bleConnectionStore.isFlashingFirmware}
				>
					Check clock drift (E2 AC)
				</button>
			</div>
		</div>
	</div>
...
<div class="collapse collapse-arrow bg-base-300 rounded-lg">
	...
	<div class="collapse-content">
		<div class="prose max-w-none p-0 flex items-start flex-col gap-3">
			...
			<div class="flex flex-wrap gap-3 mt-4 pt-4 border-t border-base-100 w-full">
				<h4 class="w-full">Custom Command (hex)</h4>
				<div class="join">
					<input
						class="input input-secondary input-bordered join-item w-32"
						id="customHex"
						value="0055"
						disabled={!bleConnectionStore.connected || bleConnectionStore.isFlashingFirmware}
					/>
					<button
						...
						onclick={() => {
							...
							if (input) {
								bleConnectionStore.sendCommand(input.value);
							}
						}}
						disabled={!bleConnectionStore.connected || bleConnectionStore.isFlashingFirmware}
						>Send EPD Cmd</button
					...
					<button
						...
						onclick={() => {
							const input = document.getElementById('customHex');
							if (input) {
								bleConnectionStore.sendRxTxCommand(input.value);
							}
						}}
						disabled={!bleConnectionStore.connected || bleConnectionStore.isFlashingFirmware}
						>Send RxTx Cmd</button
					>
				</div>
			</div>
		</div>
