...
void epd_display_time_with_date(struct date_time _time, uint16_t battery_mv, int16_t temperature, uint8_t full_or_partial)
{
    ...
    // Battery icon rectangle (main body)
    uint16_t battery_main_right = (right > 4) ? (right - 4) : right;
    obdRectangle(&obd, battery_left, 2, battery_main_right, 22, 1, 1);

    // Battery tip (positive terminal)
    obdRectangle(&obd, battery_main_right, 8, right, 16, 1, 1);

    // Battery percentage inside battery outline (drawn white on black fill)
    sprintf(buff, "%d", battery_level);
    uint16_t battery_text_adjusted = (battery_main_right > 2) ? (battery_main_right - 2) : battery_main_right;
    epd_write_text_right(&obd, (GFXfont *)&Dialog_plain_16, battery_text_left, battery_text_adjusted, 18, (char *)buff, 0);
